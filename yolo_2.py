...
class YoloLayer(Layer):
    def __init__(self, anchors, max_grid, batch_size, warmup_batches, ignore_thresh,
        ...
        self.anchors = tf.reshape(tf.tile(self.anchors,[1,1,1,1,4]),[1,1,1,12,2])
        ...
        cell_x=np.reshape(np.tile(np.arange(max_grid_w),max_grid_h),[1,max_grid_h,max_grid_w,1,1])
        cell_y=np.transpose(cell_x,[0,2,1,3,4])

        cellx=np.zeros_like(np.tile(np.reshape(cell_x,[1,max_grid_h,max_grid_w,1,1,1]),[1,1,1,1,1,4]))
        cellx[...,0]=cell_x[...]
        cellx[...,1]=cell_x[...]+1
        cellx[...,2]=cell_x[...]
        cellx[...,3]=cell_x[...]+1
        celly=np.zeros_like(np.tile(np.reshape(cell_y,[1,max_grid_h,max_grid_w,1,1,1]),[1,1,1,1,1,4]))
        celly[...,0]=cell_y[...]
        celly[...,1]=cell_y[...]
        celly[...,2]=cell_y[...]+1
        celly[...,3]=cell_y[...]+1
        cellx=np.tile(cellx,[1,1,1,3,1,1])
        celly=np.tile(celly,[1,1,1,3,1,1])
        cellx=np.transpose(cellx,[0,1,2,3,5,4])
        celly=np.transpose(celly,[0,1,2,3,5,4])
        cellx=tf.convert_to_tensor(cellx,dtype=tf.float32)
        celly=tf.convert_to_tensor(celly,dtype=tf.float32)
        self.cell_grid = tf.tile(tf.concat([cellx,celly],-1), [batch_size, 1, 1, 1, 1, 1])
        self.cell_grid = tf.reshape(self.cell_grid, [batch_size, max_grid_h, max_grid_w, 12, 2])
    ...
    def call(self, x):
        ...
        y_pred = tf.reshape(y_pred, tf.concat([tf.shape(y_pred)[:3], tf.constant([12, -1])], axis=0))
        ...
        Adjust prediction
        """
        pred_box_xy = (self.cell_grid[:, :grid_h, :grid_w, :, :] + tf.tanh(y_pred[..., :2]))  # sigma(t_xy) + c_xy
        pred_box_wh = tf.tanh(y_pred[..., 2:4])  # t_wh
...
def create_yolov3_model(
    ...
    true_yolo_1 = Input(
        shape=(None, None, len(anchors) // 6 *4, 4 + 1 + nb_class))  # grid_h, grid_w, nb_anchor, 5+nb_class
    true_yolo_2 = Input(
        shape=(None, None, len(anchors) // 6 *4, 4 + 1 + nb_class))  # grid_h, grid_w, nb_anchor, 5+nb_class
    true_yolo_3 = Input(
        shape=(None, None, len(anchors) // 6 *4, 4 + 1 + nb_class))  # grid_h, grid_w, nb_anchor, 5+nb_class
    ...
    pred_yolo_1 = _conv_block(x, [
        ...
        {'filter': (3 *4* (5 + nb_class)), 'kernel': 1, 'stride': 1, 'bnorm': False, 'leaky': False, 'layer_idx': 81}],
                              do_skip=False)
    ...
    pred_yolo_2 = _conv_block(x,
                              [{'filter': 512, 'kernel': 3, 'stride': 1, 'bnorm': True, 'leaky': True, 'layer_idx': 92},
                               {'filter': (3 *4* (5 + nb_class)), 'kernel': 1, 'stride': 1, 'bnorm': False,
